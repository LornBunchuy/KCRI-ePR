    ...
    return new Promise(function (resolve, reject) {
        setTimeout(function () {
            if ($('#xsalert').length == 0) {
                ...
                /* Icons */
                if (icon == 'success') { icon = '/lib/XSAlert/XSAlertImage/checked.png'; };
                if (icon == 'warning') { icon = '/lib/XSAlert/XSAlertImage/warning.png'; };
                if (icon == 'error') { icon = '/lib/XSAlert/XSAlertImage/cancel.png'; };
                if (icon == 'question') { icon = '/lib/XSAlert/XSAlertImage/Question.png'; };
                if (icon == 'notification') { icon = '/lib/XSAlert/XSAlertImage/notification.png'; };
                if (icon == 'like') { icon = '/lib/XSAlert/XSAlertImage/love.png'; };
                if (icon == 'thumbup') { icon = '/lib/XSAlert/XSAlertImage/like.png'; };
                if (icon == 'thumbdown') { icon = '/lib/XSAlert/XSAlertImage/dislike.png'; };
                if (icon == 'information') { icon = '/lib/XSAlert/XSAlertImage/information.png'; };

                var divImgUrl;
                var divIcon;
                var overlaydiv = document.querySelector('div');
                if (imageURL == null || imageURL == "") {
                    divImgUrl = '';
                } else {
                    divImgUrl = '<div id="xs-image" class="xs-image"><img src="' + imageURL + '" width="' + imageWidth + '" height="' + imageHeight + '"></div>'
                }
                if (icon == null || icon == "") {
                    divIcon = '';
                } else {
                    divIcon = '<div id="xs-icon" class="xs-icon jello-diagonal"><img src="' + icon + '"></div>'
                }
                if (overlaydiv.classList.contains('xsoverlay') == false) {
                    overlaydiv = '<div id="xsoverlay" class="xsoverlay"></div>';
                }
                else {
                    overlaydiv = '';
                }
                ...
                $('body').append(
                    ...
                    '<img id="xs-progress-icon" class="xs-progress-icon" src="/lib/XSAlert/XSAlertImage/loading.gif"><div id="xs-progress-bar" class="xs-progress-bar"></div>' +
                    '</div>'
                );
                ...
                if (messageAlign == null) { $('#xs-message').css('text-align', 'center'); }
                else { $('#xs-message').css('text-align', messageAlign); }
                ...
                if (autoCloseTimer == null) {
                    $("#xs-progress-bar").remove(); $("#xs-progress-icon").remove();
                } else {
                ...
                switch (inputType) {
                    ...
                    case 'range':
                        ...
                        if (inputAttributes == null) {
                            $("#xs-input").attr("min", 0); $("#xs-input").attr("max", 100); $("#xs-input").attr("step", 1);
                ...
                $('[id="xs-ok-btn"]').click(function () { closeXSAlert(); resolve('ok') });
                $("#xs-ok-btn").focus();
                /* Cancel button onClick */
                $('[id="xs-cancel-btn"]').click(function () { closeXSAlert(); resolve('cancel') });
                /* Third button onClick */
                $('[id="xs-third-btn"]').click(function () { closeXSAlert(); resolve('third') });
                /* Close with ESC key */
